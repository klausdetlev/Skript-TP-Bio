...
\begin{figure}[H]
	...
	\begin{tikzpicture}[>=stealth]
		...
		\draw (2,4)--(2,2)--(5,2);
		\draw (0,0.6)node[left]{$\Omega$}--(1.6,2)--(2,1.6)--(0.6,0)node[below]{$\Omega$};
		...
		\draw[->] (lmh) -- (1.9,1.9);
		\draw[->] (s) .. controls +(2,-1) and +(-2,0.5) .. ++(-1,-3);
		\begin{scope}[xshift=1cm,yshift=-5cm]
			\draw[<->] (0,3)node[above]{Shock profile}--(0,0)--(3,0)node[below right]{$x$};
			\foreach \x \y in {0.6/\alpha,1.5/\frac{1}{2},2.4/1-\beta}{
				\draw (-0.1,\x)node[left]{$\y$}--++(0.2,0);
			}
			\draw (2.4,-0.1)node[below]{1}--++(0,0.2);
			\draw (0,0.6)--(1.2,1.2)(1.2,1.8)--(2.4,2.4);
			\draw[densely dashed] (1.2,1.2)--(1.2,1.8);
		\end{scope}
		\begin{scope}[xshift=6cm,yshift=1cm]
			\draw[<->] (3,0)node[below right]{$\rho$}--(0,0)--(0,3)node[above left]{$J(\rho)$};
			\node at (2,4){$J(\rho)=\rho(1-\rho)$};
			\draw (0,0) parabola bend (1.2,2.4) (2.4,0);
			\draw (1.2,-0.1)node[below]{$\frac{1}{2}$}--++(0,0.2);
			\draw (-0.1,2.4)node[left]{$\frac{1}{4}$}--++(0.2,0);
			\draw[densely dashed] (1.2,0)--(1.2,2.4)(0.8,2)--(1.6,2);
		\end{scope}
	\end{tikzpicture}
\end{figure}
